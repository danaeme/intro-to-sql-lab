...
WHERE countries.region = 'Southern Europe' AND countrylanguages.isofficial = TRUE 
...
SELECT newcountry.name
FROM countries oldcountry
JOIN countrylanguages oldcountrylanguage ON oldcountry.code = oldcountrylanguage.countrycode
JOIN countrylanguages newcountrylanguage ON oldcountrylanguage.language = newcountrylanguage.language
JOIN countries newcountry ON newcountry.code = newcountrylanguage.countrycode
WHERE oldcountry.name = 'Holy See (Vatican City State)'  
AND oldcountrylanguage.language = 'Italian'  
AND newcountrylanguage.isofficial = TRUE
GROUP BY newcountry.name
HAVING COUNT(newcountrylanguage.language) = 1;

-- Vatican City, Italy, San Marino, Switzerland
